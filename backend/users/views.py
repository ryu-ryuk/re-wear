...
def request_password_reset(request):
    ...
    Request a password reset link. The user provides their email,
    and if it exists, a reset link is sent to their email.
...
def reset_password(request, uidb64, token):
    """
    🔑 POST /api/users/reset_password/{uid}/{token}/
    
    Reset the password for a user using the token sent via email.
    ...
    {
        "message": "Invalid token or uid. Please request a new password reset link."
    }
    """
    try:
